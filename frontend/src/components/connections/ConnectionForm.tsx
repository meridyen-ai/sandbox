 ...
 * Similar to ThoughtSpot's connection configuration UI.
 */
...
export const ConnectionForm: React.FC<ConnectionFormProps> = ({
  ...
  const [connectionName, setConnectionName] = useState(existingConnectionName || "");
  ...
  useEffect(() => {
    // Load connection args from handler
    const args = handler.connection_args || [];
    setConnectionArgs(args);

    // Initialize form values with defaults from handler
    const initialValues: Record<string, string | number | boolean> = {};
    args.forEach((arg) => {
      // Use default from handler if available
    ...
    // Reset test result and error when handler changes
    setTestResult(null);
    setError(null);
  }, [handler]);
  ...
  const validateForm = (): boolean => {
    if (!connectionName.trim()) {
      setError(t("dataSources.errors.nameRequired") || "Connection name is required");
      return false;
    }
    ...
    for (const arg of connectionArgs) {
      ...
      if (arg.required && isFieldVisible(arg)) {
        ...
        if (value === undefined || value === "" || value === null) {
          setError(t("dataSources.errors.fieldRequired", { field: arg.label }) || `${arg.label} is required`);
          return false;
        }
      }
    }
  ...
  const renderField = (arg: ConnectionArg) => {
    // Check visibility based on depends_on
    ...
    if (arg.type === "boolean") {
      return (
        <div key={arg.name} className="flex items-center justify-between py-3 border-b border-slate-100 dark:border-dashboard-border last:border-0">
          <div>
    ...
    if (arg.type === "text" || arg.name.includes("json") || arg.name.includes("credentials") || arg.name.includes("private_key")) {
      return (
        <div key={arg.name} className="space-y-1.5">
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">
            {arg.label}
    ...
    return (
      <div key={arg.name} className="space-y-1.5">
        ...
        <div className="relative">
          <input
            type={isSecret && !showSecret ? "password" : arg.type === "integer" ? "number" : "text"}
            value={value as string}
            onChange={(e) => handleInputChange(arg.name, e.target.value)}
            placeholder={arg.description}
  ...
  // Group fields: required fields first, then optional
  const requiredFields: ConnectionArg[] = connectionArgs.filter((arg: ConnectionArg) => arg.required);
  const optionalFields: ConnectionArg[] = connectionArgs.filter((arg: ConnectionArg) => !arg.required);
    ...
    <div className="min-h-full flex flex-col">
      ...
      <div className="mb-6">
        ...
        <h2 className="text-xl font-semibold text-slate-900 dark:text-white">
          {t("dataSources.nameAndConfigure") || "Name and configure your connection"}
        </h2>
      </div>
      ...
      <div className="flex-1 space-y-6">
        ...
        <div className="space-y-4">
          <h3 className="text-sm font-medium text-slate-500 dark:text-slate-400">
            {t("dataSources.nameAndDescribe") || "Name and describe the connection"}
          </h3>
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder={t("dataSources.connectionNamePlaceholder") || "e.g., production-db, analytics-warehouse"}
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                placeholder={t("dataSources.descriptionPlaceholder") || "Describe this connection..."}
                rows={2}
