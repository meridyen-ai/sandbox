"""Amazon Aurora MySQL database handler implementation."""
...
class AuroraMySQLHandler(MetaDatabaseHandler):
    """
    Handler for Amazon Aurora MySQL-compatible databases.

    Supports querying, schema introspection, and CRUD operations
    on Aurora MySQL databases. Aurora MySQL is wire-compatible with MySQL,
    so this handler uses the MySQL connector.
    """

    handler_name = "aurora_mysql"
    handler_type = HandlerType.DATA
    handler_title = "Amazon Aurora MySQL"
    handler_description = "Connect to Amazon Aurora MySQL-compatible databases"
    handler_version = "0.1.0"

    dialect = "mysql"

    # Map MySQL types to canonical types (Aurora MySQL uses MySQL types)
    type_mapping = {
    ...
    def __init__(self, name: str, connection_args: dict):
        """
        Initialize Aurora MySQL handler.
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            # Handle SSL (recommended for Aurora)
            ssl = self.connection_args.get("ssl", True)
            if ssl:
            ...
            logger.info(f"Connected to Aurora MySQL: {self.name}")

        except MySQLError as e:
            self.is_connected = False
            logger.error(f"Failed to connect to Aurora MySQL: {e}")
            raise ConnectionError(f"Failed to connect to Aurora MySQL: {e}")

    def disconnect(self) -> None:
        """Close the Aurora MySQL connection."""
        if self._connection:
            try:
                ...
                self.is_connected = False
                logger.info(f"Disconnected from Aurora MySQL: {self.name}")

    def check_connection(self) -> HandlerStatus:
        """Check if the Aurora MySQL connection is working."""
        try:
