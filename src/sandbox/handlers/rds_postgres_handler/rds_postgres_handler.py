"""Amazon RDS PostgreSQL database handler implementation."""
...
class RDSPostgresHandler(MetaDatabaseHandler):
    """
    Handler for Amazon RDS PostgreSQL databases.

    Supports querying, schema introspection, and CRUD operations
    on RDS PostgreSQL databases.
    """

    handler_name = "rds_postgres"
    handler_type = HandlerType.DATA
    handler_title = "Amazon RDS PostgreSQL"
    handler_description = "Connect to Amazon RDS PostgreSQL databases"
    ...
    def __init__(self, name: str, connection_args: dict):
        """
        Initialize RDS PostgreSQL handler.
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            logger.info(f"Connected to RDS PostgreSQL: {self.name}")

        except Exception as e:
            self.is_connected = False
            logger.error(f"Failed to connect to RDS PostgreSQL: {e}")
            raise ConnectionError(f"Failed to connect to RDS PostgreSQL: {e}")

    def disconnect(self) -> None:
        """Close the RDS PostgreSQL connection."""
        if self._connection:
            try:
                ...
                self.is_connected = False
                logger.info(f"Disconnected from RDS PostgreSQL: {self.name}")

    def check_connection(self) -> HandlerStatus:
        """Check if the RDS PostgreSQL connection is working."""
        try:
