"""SQL Server database handler implementation."""
...
class SQLServerHandler(MetaDatabaseHandler):
    """Handler for Microsoft SQL Server databases."""

    handler_name = "sqlserver"
    handler_type = HandlerType.DATA
    handler_title = "SQL Server"
    handler_description = "Connect to Microsoft SQL Server databases"
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            trust_cert = "yes" if self.connection_args.get("trust_server_certificate", False) else "no"
            ...
            logger.info(f"Connected to SQL Server: {self.name}")
    ...
    def disconnect(self) -> None:
        ...
        if self._connection:
            try:
                ...
                self.is_connected = False
                logger.info(f"Disconnected from SQL Server: {self.name}")

    def check_connection(self) -> HandlerStatus:
        """Check if the SQL Server connection is working."""
        try:
