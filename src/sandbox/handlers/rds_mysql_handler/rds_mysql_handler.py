"""Amazon RDS MySQL database handler implementation."""
...
class RDSMySQLHandler(MetaDatabaseHandler):
    """
    Handler for Amazon RDS MySQL databases.

    Supports querying, schema introspection, and CRUD operations
    on RDS MySQL databases.
    """

    handler_name = "rds_mysql"
    handler_type = HandlerType.DATA
    handler_title = "Amazon RDS MySQL"
    handler_description = "Connect to Amazon RDS MySQL databases"
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            # Handle SSL (recommended for RDS)
            ...
            logger.info(f"Connected to RDS MySQL: {self.name}")

        except MySQLError as e:
            self.is_connected = False
            logger.error(f"Failed to connect to RDS MySQL: {e}")
            raise ConnectionError(f"Failed to connect to RDS MySQL: {e}")

    def disconnect(self) -> None:
        """Close the RDS MySQL connection."""
        if self._connection:
            try:
                ...
                self.is_connected = False
                logger.info(f"Disconnected from RDS MySQL: {self.name}")

    def check_connection(self) -> HandlerStatus:
        """Check if the RDS MySQL connection is working."""
        try:
